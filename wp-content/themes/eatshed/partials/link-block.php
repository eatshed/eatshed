<section class="link-block container container-fluid">
    <ul class="link-block__list columns is-mobile is-multiline">
        <li class="link-block__item column is-half-mobile">
            <a href="<?php echo home_url(); ?>/shed-whole-foods-plant-based-restaurant-cutlery-works-sheffield/" class="link-block__link">
                <img class="link-block__icon" src="<?php echo get_template_directory_uri() ?>/images/link-venues.svg" alt="See our venues" />
                <img class="link-block__icon link-block__icon--hover" src="<?php echo get_template_directory_uri() ?>/images/link-venues-hover.svg" alt="See our venues" />
            </a>
        </li>
        <li class="link-block__item column is-half-mobile">
            <a href="<?php echo home_url(); ?>/wp-content/uploads/menu.pdf" class="link-block__link">
                <img class="link-block__icon" src="<?php echo get_template_directory_uri() ?>/images/link-menu.svg" alt="View our menu" />        
                <img class="link-block__icon link-block__icon--hover" src="<?php echo get_template_directory_uri() ?>/images/link-menu-hover.svg" alt="View our menu" />        
            </a>
        </li>
        <li class="link-block__item column is-half-mobile">
            <a href="<?php echo home_url(); ?>/events" class="link-block__link">
                <img class="link-block__icon" src="<?php echo get_template_directory_uri() ?>/images/link-events.svg" alt="Find out about our events" />
                <img class="link-block__icon link-block__icon--hover" src="<?php echo get_template_directory_uri() ?>/images/link-events-hover.svg" alt="Find out about our events" />
            </a>
        </li>
        <li class="link-block__item column is-half-mobile">
            <a href="<?php echo home_url(); ?>/nicely-nourished-plant-based-blog" class="link-block__link">
                <img class="link-block__icon" src="<?php echo get_template_directory_uri() ?>/images/link-blog.svg" alt="Read our blog" />
                <img class="link-block__icon link-block__icon--hover" src="<?php echo get_template_directory_uri() ?>/images/link-blog-hover.svg" alt="Read our blog" />
            </a>
        </li>
    </ul>
</section>